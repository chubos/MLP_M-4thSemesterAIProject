...
ax.set_title('Średnia PK w funkcji liczby neuronów (K1, K2)', fontsize=12)
...
ax.set_title('Średnia PK w funkcji parametrów uczenia (lr, mc)', fontsize=12)
...
ax.axvline(best['PK'], color='red', linestyle='--', linewidth=2, label=f'Najlepszy: {best["PK"]:.2f}%')
ax.set_xlabel('PK [%]', fontsize=12)
ax.set_ylabel('Liczba kombinacji', fontsize=12)
ax.set_title('Rozkład skuteczności dla wszystkich kombinacji', fontsize=12)
...
ax.set_title('Wpływ liczby neuronów K1 na skuteczność', fontsize=12)
ax.grid(True, alpha=0.3)
plt.tight_layout()
...
ax.set_xlabel('K2', fontsize=12)
ax.set_ylabel('PK [%]', fontsize=12)
ax.set_title('Wpływ liczby neuronów K2 na skuteczność', fontsize=12)
ax.grid(True, alpha=0.3)
plt.tight_layout()
...
top_10_pk = [r['PK'] for r in results_sorted[:10]]
top_10_labels = [f"K1={r['K1']}\nK2={r['K2']}\nlr={r['lr']:.0e}\nmc={r['mc']:.2f}" for r in results_sorted[:10]]
colors = ['green' if i == 0 else 'steelblue' for i in range(10)]
ax.barh(range(len(top_10_pk)), top_10_pk, color=colors, edgecolor='black')
ax.set_yticks(range(len(top_10_pk)))
ax.set_yticklabels(top_10_labels, fontsize=9)
ax.set_xlabel('PK [%]', fontsize=12)
ax.set_title('Top 10 najlepszych kombinacji parametrów', fontsize=12)
ax.invert_yaxis()
for i, v in enumerate(top_10_pk):
    ax.text(v - 0.3, i, f'{v:.2f}%', va='center', ha='right', fontweight='bold', fontsize=10)
plt.tight_layout()
plt.savefig('Fig.5f_GridSearch_top10.png', dpi=150, bbox_inches='tight')
print("✓ Fig.5f zapisany")
plt.close()
