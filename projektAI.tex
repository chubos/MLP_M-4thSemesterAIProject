...
przyjęto \texttt{K1 = 8} oraz \texttt{K2 = 5}.

\subsection{Eksperyment nr 2}

W eksperymencie badano wpływ parametrów uczenia na skuteczność klasyfikacji,
przy ustalonej architekturze z eksperymentu nr 1 (\texttt{K1 = 8}, \texttt{K2 = 5}).
...
\begin{itemize}[label={\textbullet}]
\item stała architektura sieci: \texttt{K1 = 8}, \texttt{K2 = 5},
...
Na podstawie uzyskanych wyników otrzymano optymalną konfigurację parametrów uczenia: \texttt{lr = 0.0001} oraz \texttt{mc = 0.80}.
...
Analiza wykresu pokazuje, że wpływ cech na decyzję sieci nie jest równomierny. Największy spadek skuteczności wystąpił dla kwestii \texttt{physician-fee-freeze} (około \(4.2\%\)), co wskazuje na dużą zawartość informacyjną. Istotne, ale mniejsze spadki obserwowano także dla \texttt{synfuels-cutback} (około \(1.2\%\)) oraz \texttt{el-salvador-aid} (około \(0.9\%\)).
