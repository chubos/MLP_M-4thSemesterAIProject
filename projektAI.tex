...
\begin{equation}
E = SSE = \frac{1}{2}\sum_{i_3=1}^{K_3}\left(y_{i_3}^{(3)}-\hat{y}_{i_3}^{(3)}\right)^2.
\end{equation}
...
f^{(1)}\!\left(\sum_{j=1}^{L} w_{i_1 j}^{(1)}x_j + b_{i_1}^{(1)}\right)+ b_{i_2}^{(2)}\right)
+ b_{i_3}^{(3)}\right)
-\hat{y}_{i_3}^{(3)}\right)^2.
\end{equation}
...
= \frac{\partial E}{\partial y_{i_3}^{(3)}}
\frac{\partial y_{i_3}^{(3)}}{\partial z_{i_3}^{(3)}}
\frac{\partial z_{i_3}^{(3)}}{\partial w^{(3)}_{i_3 i_2}}
= \left(y_{i_3}^{(3)}-\hat{y}_{i_3}^{(3)}\right)
f'^{(3)}\!\left(z_{i_3}^{(3)}\right)
y_{i_2}^{(2)}.
\end{equation}
...
\frac{\partial E}{\partial y_{i_3}^{(3)}}
\frac{\partial y_{i_3}^{(3)}}{\partial z_{i_3}^{(3)}}
\frac{\partial z_{i_3}^{(3)}}{\partial y_{i_2}^{(2)}}
\frac{\partial y_{i_2}^{(2)}}{\partial z_{i_2}^{(2)}}
\frac{\partial z_{i_2}^{(2)}}{\partial w^{(2)}_{i_2 i_1}} \\
&= \sum_{i_3=1}^{K_3}
\left(y_{i_3}^{(3)}-\hat{y}_{i_3}^{(3)}\right)
f'^{(3)}\!\left(z_{i_3}^{(3)}\right)
w^{(3)}_{i_3 i_2}
f'^{(2)}\!\left(z_{i_2}^{(2)}\right)
y_{i_1}^{(1)}.
\end{aligned}
\end{equation}
...
\frac{\partial z_{i_3}^{(3)}}{\partial y_{i_2}^{(2)}}
\frac{\partial y_{i_2}^{(2)}}{\partial z_{i_2}^{(2)}}
\frac{\partial z_{i_2}^{(2)}}{\partial y_{i_1}^{(1)}}
\frac{\partial y_{i_1}^{(1)}}{\partial z_{i_1}^{(1)}}
\frac{\partial z_{i_1}^{(1)}}{\partial w^{(1)}_{i_1 j}} \\
&= \sum_{i_3=1}^{K_3}
\left(y_{i_3}^{(3)}-\hat{y}_{i_3}^{(3)}\right)
f'^{(3)}\!\left(z_{i_3}^{(3)}\right)
\sum_{i_2=1}^{K_2}
...
w^{(2)}_{i_2 i_1}
f'^{(2)}\!\left(z_{i_2}^{(2)}\right)
f'^{(1)}\!\left(z_{i_1}^{(1)}\right)
x_j.
\end{aligned}
\end{equation}

W celu skrócenia zapisu wprowadza się sygnały błędu dla kolejnych warstw:
\begin{equation}
\begin{aligned}
\delta^{(3)}_{i_3} &= \left(y_{i_3}^{(3)}-\hat{y}_{i_3}^{(3)}\right)\,f'^{(3)}\!\left(z_{i_3}^{(3)}\right),\\
\delta^{(2)}_{i_2} &= \left(\sum_{i_3=1}^{K_3} w^{(3)}_{i_3 i_2}\,\delta^{(3)}_{i_3}\right)\,f'^{(2)}\!\left(z_{i_2}^{(2)}\right),\\
\delta^{(1)}_{i_1} &= \left(\sum_{i_2=1}^{K_2} w^{(2)}_{i_2 i_1}\,\delta^{(2)}_{i_2}\right)\,f'^{(1)}\!\left(z_{i_1}^{(1)}\right).
\end{aligned}
\end{equation}

Wtedy pochodne względem wag i biasów mają zwarte postacie:
\begin{equation}
\frac{\partial E}{\partial w^{(l)}_{i_l i_{l-1}}} = \delta^{(l)}_{i_l}\,y_{i_{l-1}}^{(l-1)},
\qquad
\frac{\partial E}{\partial b^{(l)}_{i_l}} = \delta^{(l)}_{i_l}.
\end{equation}

Aktualizację parametrów realizuje się metodą największego spadku:
\begin{equation}
w^{(l)}_{i_l i_{l-1}}(t+1)=w^{(l)}_{i_l i_{l-1}}(t)-\eta\,\frac{\partial E}{\partial w^{(l)}_{i_l i_{l-1}}}(t),
\qquad
b^{(l)}_{i_l}(t+1)=b^{(l)}_{i_l}(t)-\eta\,\frac{\partial E}{\partial b^{(l)}_{i_l}}(t).
\end{equation}
...
W praktyce często przyjmuje się $m_c\approx 0{,}9$--$0{,}95$, co zwykle zwiększa szybkość uczenia i ogranicza oscylacje podczas minimalizacji funkcji celu \cite{zajdelLabm}.
...
Głównym komponentem odpowiedzialnym za architekturę i logikę sieci neuronowej jest moduł opisany listingiem \ref{lst:mlp_core_py}, w którym zdefiniowano klasę implementującą sieć MLP. Zawiera ona strukturę wielowarstwowego perceptronu składającą się z jednej warstwy wejściowej, jednej warstwy ukrytej oraz jednej warstwy wyjściowej. Moduł ten zapewnia środowisko do pracy z modelem, obejmujące procesy inicjalizacji wag i~biasów oraz algorytmy uczenia oparte na wstecznej propagacji błędu z mechanizmem momentum. Całość implementacji została zoptymalizowana pod kątem obliczeniowym poprzez wykorzystanie zewnętrznych funkcji pomocniczych, które wspierają operacje matematyczne i aktualizację parametrów sieci.
...
Przykładowe uruchomienie programu wykorzystuje sieć o parametrach $K_1=4$, $K_2=4$, współczynniku uczenia $lr=10^{-3}$, współczynniku momentum $mc=0{,}9$, progu błędu $err\_goal=1$ oraz maksymalnie $400$ epokach uczenia. Zbiór danych został podzielony na część treningową i testową w proporcji $80/20$. Przebieg wartości $SSE$ w~kolejnych epokach przedstawia rysunek \ref{fig:sse_test}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{Fig.1_PK_K1K2_kongres.png}
...
przyjęto \texttt{K1 = 5} oraz \texttt{K2 = 6}.

Uzyskano maksymalną skuteczność na poziomie 96,31\%.

\subsection{Eksperyment nr 2}

W eksperymencie badano wpływ parametrów uczenia na skuteczność klasyfikacji,
przy ustalonej architekturze z eksperymentu nr 1 (\texttt{K1 = 5}, \texttt{K2 = 6}).
...
\begin{itemize}[label={\textbullet}]
\item stała architektura sieci: \texttt{K1 = 5}, \texttt{K2 = 6},
\item stałe warunki uczenia: \texttt{max\_epoch = 400}, \texttt{err\_goal = 1},
\item przeszukiwana siatka parametrów: $lr \in [10^{-5}, 10^{-1}]$ (skala logarytmiczna),
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{Fig.2_PK_lrmc_kongres.png}
...
Na podstawie uzyskanych wyników otrzymano optymalną konfigurację parametrów uczenia: \texttt{lr = 0.0001} oraz \texttt{mc = 0.25}.
...
\item przeszukiwane parametry uczenia: $lr \in [10^{-5}, 10^{-2}]$,
...
numbers=left,
frame=single,
caption={Eksperyment nr 5: pe{\l}ny GridSearch po czterech parametrach},
label={lst:exp5_gridsearch_4d_py}
]{exp5_gridsearch_4d.py}

GridSearch sprawdza każdą kombinację przy użyciu walidacji krzyżowej. Dla każdej kombinacji zapisywane są uzyskane wyniki, które następnie są sortowane po skuteczności. Wyniki są wizualizowane z różnych perspektyw, co pozwala zrozumieć wpływ poszczególnych parametrów na jakość klasyfikacji.


Rysunek \ref{fig:exp5_k1k2} pokazuje wpływ architektury sieci na skuteczność poprzez uśrednienie wyników dla wszystkich kombinacji parametrów uczenia. Pokazuje, które pary neuronów (K1, K2) osiągają najwyższą średnią skuteczność niezależnie od parametrów lr i mc. Analiza tego wykresu pozwala zidentyfikować optymalny zakres rozmiarów warstw ukrytych.
...
Rysunek \ref{fig:exp5_histogram} prezentuje rozkład statystyczny skuteczności dla wszystkich sprawdzonych kombinacji. Analiza tego rozkładu pozwala ocenić, czy większość konfiguracji daje dobre rezultaty, czy też tylko nieliczne kombinacje osiągają wysoką skuteczność, co wskazywałoby na wąskie optimum w przestrzeni hiperparametrów.
...
Rysunek \ref{fig:exp5_k1_scatter} przedstawia rozkład punktowy wszystkich kombinacji względem K1. Dla każdej wartości K1 pokazane są wszystkie możliwe skuteczności osiągane dla różnych kombinacji pozostałych trzech parametrów. Ten wykres ujawnia, czy liczba neuronów w pierwszej warstwie ukrytej ma duży wpływ na wynik, czy też wpływ jest silnie zależny od innych parametrów.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{Fig.5d_GridSearch_K1_scatter.png}
\caption{Wpływ liczby neuronów K1 na skuteczność}
\label{fig:exp5_k1_scatter}
\end{figure}

Rysunek \ref{fig:exp5_k2_scatter} analogicznie do poprzedniego pokazuje rozkład skuteczności w~funkcji K2 dla wszystkich kombinacji pozostałych parametrów. Porównanie tego wykresu pozwala ocenić, czy druga warstwa ukryta ma podobny czy różny wpływ na skuteczność w porównaniu z pierwszą warstwą.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{Fig.5e_GridSearch_K2_scatter.png}
\caption{Wpływ liczby neuronów K2 na skuteczność}
\label{fig:exp5_k2_scatter}
\end{figure}

\FloatBarrier

Najlepszą konfigurację uzyskano dla \texttt{K1 = 7}, \texttt{K2 = 1}, \texttt{lr = 0.0001} oraz \texttt{mc = 0.35}. Dla tej kombinacji model osiągnął skuteczność \texttt{PK = 97,01\%}, co stanowi najlepszy wynik całego eksperymentu.

\section{Podsumowanie i wnioski}

W projekcie zbudowano i przeanalizowano sieć MLP do klasyfikacji głosowań kongresowych, a uzyskane wyniki pokazały, że model osiąga bardzo wysoką skuteczność przy odpowiednio dobranych hiperparametrach. Najlepszy rezultat wyniósł \texttt{97,01\%} i został osiągnięty dla konfiguracji \texttt{K1 = 7}, \texttt{K2 = 1}, \texttt{lr = 0.0001} oraz \texttt{mc = 0.35}. Oznacza to, że dla tego zadania korzystniejsza jest architektura z większą pierwszą warstwą i małą drugą warstwą.

Wyniki eksperymentów pokazały też, że sieć bardzo często osiągała wysokie wartości skuteczności, a histogram rozkładu wyników był wyraźnie skupiony, a nie rozrzucony. Taki obraz sugeruje, że model nie jest silnie podatny na drobne zmiany parametrów i zachowuje stabilność w szerokim obszarze przestrzeni hiperparametrów. Całość potwierdza więc, że dobrze dobrany MLP może być skutecznym i odpornym na zakłócenia rozwiązaniem dla tego problemu klasyfikacji.
...
\bibitem{zajdelLab} Zajdel R.: Sztuczna inteligencja, Laboratorium. Sieć neuronowa jednokierunkowa jednowarstwowa. \url{https://materialy.kia.prz.edu.pl/pracownik/pliki/34/%C4%86WICZENIE%208.1.pdf}. Dostęp 21.03.2026.
\bibitem{zajdelLab9} Zajdel R.: Sztuczna inteligencja, Laboratorium. Sieć neuronowa jednokierunkowa wielowarstwowa. \url{https://materialy.kia.prz.edu.pl/pracownik/pliki/34/%C4%86WICZENIE%209.pdf}. Dostęp 21.03.2026.
\bibitem{zajdelLabm} Zajdel R.: Sztuczna inteligencja, Laboratorium. Przyśpieszanie procesu uczenia. \url{https://materialy.kia.prz.edu.pl/pracownik/pliki/34/ĆWICZENIE%2010.pdf}. Dostęp 21.03.2026.
\end{thebibliography}
