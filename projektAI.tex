...
\subsection{Opis działania kodu}
Fundamentem działania klasy jest proces inicjalizacji realizowany w metodzie \texttt{\_\_init\_\_}. Konfiguruje ona kluczowe hiperparametry sieci, takie jak rozmiary warstw ukrytych ($K_1$, $K_2$), współczynnik uczenia (\textit{learning rate}) oraz współczynnik momentum. 

Istotnym aspektem jest tutaj mechanizm zarządzania wagami. System pozwala na ich losową inicjalizację lub odczytanie uprzednio wytrenowanych wartości, co zapewnia powtarzalność eksperymentów. Za proces wnioskowania odpowiada metoda \texttt{predict}, która realizuje pełną propagację sygnału w przód. W architekturze tej zarówno obie warstwy ukryte, jak i warstwa wyjściowa, wykorzystują funkcję aktywacji \texttt{tansig} (tangens hiperboliczny). Dzięki temu sieć generuje wektor odpowiedzi $y_3$ o wartościach znormalizowanych w przedziale $(-1, 1)$. 
...
W celu obiektywnej oceny zdolności generalizacyjnych modelu zaimplementowano metodę \texttt{train\_CV}. Wykorzystuje ona strategię warstwowej walidacji krzyżowej (\textit{StratifiedKFold}), dzieląc zbiór danych na określoną liczbę podzbiorów (foldów) z zachowaniem proporcji klas. 
...
numbers=left,
frame=single,
caption={Modu{\l} obliczeń matematycznych},
