...
\begin{equation}
E = \frac{1}{2}\sum_{i_3=1}^{K_3}\left(\hat{y}^{(3)}_{i_3}-y^{(3)}_{i_3}\right)^2.
\end{equation}
...
= \left(\hat{y}^{(3)}_{i_3}-y^{(3)}_{i_3}\right)
...
numbers=left,
frame=single,
caption={Modu{\l} funkcji numerycznych sieci neuronowej \texttt{nnet\_jit4.py}},
...
numbers=left,
frame=single,
caption={Skrypt eksperymentu nr 3: analiza wa\.zno\'sci g{\l}osowa\'n (\texttt{exp3\_waznosc\_glosowan.py})},
...
numbers=left,
frame=single,
caption={Skrypt eksperymentu nr 4: wp{\l}yw liczby danych ucz\k{a}cych (\texttt{exp4\_ilosc\_danych.py})},
